...
for line in $file; do
	if [[ ! -z $(echo "${line}" | grep -i http) ]]; then
		...
		if [[ ! -z $(echo "${line}" | grep -w "IIS") ]]; then
			pages=".html,.asp,.aspx,.php,.txt,.exe"
		else
...
if [[ ! -z $(echo "${file}" | grep -w "389/tcp") ]]; then
        ...
        echo "nmap -Pn -p 389 --script ldap-search --script-args 'ldap.username=\"\$(cat recon/ldapsearch_$1.txt | grep rootDomainNamingContext | cut -d \\" \\" -f2)\"' $1 -oA recon/nmap_ldap_$1.txt"
	echo ""
fi
